# modules/webapp/main.tf (This is the content you need to replace in that specific file)
...
resource "azurerm_linux_web_app" "webapp" {
  ...
  site_config {
    always_on           = true # Recommended to keep 'always_on' for Java apps
    minimum_tls_version = var.minimum_tls_version

    # app_settings MUST be inside the site_config block
    app_settings = {
    ...
    # app_command_line MUST be directly inside the site_config block, not inside application_stack
